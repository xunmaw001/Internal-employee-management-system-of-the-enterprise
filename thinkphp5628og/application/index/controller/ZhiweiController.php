...
class ZhiweiController extends CommonController
{
    ...
    public function page(){
        ...
        $count = Db::table('zhiwei')->where($where)->where(function($query) use ($orwhere) {
            $query->whereOr($orwhere);
        })->count();
        // 取整函数(ceil,floor,round)
        $page_count = ceil($count/$limt);//页数

                $result = Db::table('zhiwei')->where($where)->where(function($query) use ($orwhere) {
            $query->whereOr($orwhere);
    ...
    public function save(){
		...
		$postData = $v;
                        $result = Db::table('zhiwei')->insert($postData);
                return json(['code'=>0]);
    }
    /**
     * 保存接口 post
    ...
    public function update(){
        ...
        Db::table('zhiwei')->where($where)->update($v);
                return json(['code'=>0]);
    }
    /**
     * 删除接口 post
    ...
    public function delete(){
        $ids = input('post.');
        $result = Db::table('zhiwei')->delete($ids);
        return json(["code"=> 0]);
    }
    /**
     * 详情接口info ,后台接口
    ...
    public function info($id=false,$name=false){
        ...
        $result = Db::table('zhiwei')->where($where)->find();
    ...
    public function remind($columnName,$type,$remindstart = false,$remindend = false){
        if ($type==1){
            ...
            $result = Db::table('zhiwei')->where($map)->count();
        }else{
    ...
    public function value($xColumnName,$yColumnName,$timeStatType){
        ...
        if(empty($timeStatType)) {
            $data = Db::query("select ".$xColumnName.", sum(".$yColumnName.") total from zhiwei group by ".$xColumnName);
            foreach($data as $k => &$v) {
            ...
            if ($timeStatType == '日') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM zhiwei ".$where." GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
            }
            if ($timeStatType == '月') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM zhiwei ".$where." GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
            }
            if ($timeStatType == '年') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM zhiwei ".$where." GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
            }
            $data = Db::query($sql);
            foreach($data as $k => &$v) {
                $v['total'] = intval($v['total']);
    ...
    public function group($columnName) {
        $list = Db::table('zhiwei')->field($columnName.',count('.$columnName.') as total')->group($columnName)->order('total desc')->select();
