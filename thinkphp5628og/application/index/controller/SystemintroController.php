...
class SystemintroController extends CommonController
{
    ...
    public function page(){
        ...
        $count = Db::table('systemintro')->where($where)->where(function($query) use ($orwhere) {
            $query->whereOr($orwhere);
        })->count();
        // 取整函数(ceil,floor,round)
        $page_count = ceil($count/$limt);//页数

                $result = Db::table('systemintro')->where($where)->where(function($query) use ($orwhere) {
            $query->whereOr($orwhere);
    ...
    public function save(){
		...
		$postData = $v;
                        $result = Db::table('systemintro')->insert($postData);
                return json(['code'=>0]);
    }
    /**
     * 保存接口 post
    ...
    public function update(){
        ...
        Db::table('systemintro')->where($where)->update($v);
                return json(['code'=>0]);
    }
    /**
     * 删除接口 post
    ...
    public function delete(){
        $ids = input('post.');
        $result = Db::table('systemintro')->delete($ids);
        return json(["code"=> 0]);
    }
    /**
     * 详情接口info ,后台接口
    ...
    public function info($id=false,$name=false){
        ...
        $result = Db::table('systemintro')->where($where)->find();
    ...
    public function remind($columnName,$type,$remindstart = false,$remindend = false){
        if ($type==1){
            ...
            $result = Db::table('systemintro')->where($map)->count();
        }else{
    ...
    public function value($xColumnName,$yColumnName,$timeStatType){
        ...
        if(empty($timeStatType)) {
            $data = Db::query("select ".$xColumnName.", sum(".$yColumnName.") total from systemintro group by ".$xColumnName);
            foreach($data as $k => &$v) {
    ...
    public function group($columnName) {
        $list = Db::table('systemintro')->field($columnName.',count('.$columnName.') as total')->group($columnName)->order('total desc')->select();
