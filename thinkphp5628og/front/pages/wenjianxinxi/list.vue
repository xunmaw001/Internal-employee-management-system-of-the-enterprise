...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
			...
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.wenjianmingcheng" type="text" placeholder="文件名称" ></input>
					</view>
					<view v-if="queryIndex==1" class="search-form round">
						<text class="cuIcon-search"></text>
						<input v-model="searchForm.wenjianriqi" type="text" placeholder="文件日期" ></input>
					</view>
				<view class="action">
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"2rpx solid #d8f0e7","boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 0 40rpx","backgroundColor":"#e9f8f1","borderRadius":"16rpx","flexWrap":"wrap","display":"flex","width":"48%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"0 0 8rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenjianxinxi','修改')) || (!userid && isAuthFront('wenjianxinxi','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenjianxinxi','删除')) || (!userid && isAuthFront('wenjianxinxi','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 0px 0px rgba(0,0,0,.3)","color":"#666","bottom":"40rpx","right":"8rpx","outline":"none","borderRadius":"100%","background":"#c8e3c3","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('wenjianxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 0px 0px rgba(0,0,0,.3)","color":"#666","bottom":"40rpx","right":"8rpx","outline":"none","borderRadius":"100%","background":"#c8e3c3","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('wenjianxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.wenjianmingcheng="";
				this.searchForm.wenjianriqi="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.wenjianmingcheng){
					params['wenjianmingcheng'] = '%' + this.searchForm.wenjianmingcheng + '%'
				}
				if(this.searchForm.wenjianriqi){
					params['wenjianriqi'] = '%' + this.searchForm.wenjianriqi + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`wenjianxinxi`, params);
                } else {
                    res = await this.$api.list(`wenjianxinxi`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('wenjianxinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.wenjianmingcheng){
					searchForm['wenjianmingcheng'] = '%' + this.searchForm.wenjianmingcheng + '%'
				}
				if(this.searchForm.wenjianriqi){
					searchForm['wenjianriqi'] = '%' + this.searchForm.wenjianriqi + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`wenjianxinxi`, searchForm);
                } else {
                    res = await this.$api.list(`wenjianxinxi`, searchForm);
                }
				// 如果是第一页数据置空
